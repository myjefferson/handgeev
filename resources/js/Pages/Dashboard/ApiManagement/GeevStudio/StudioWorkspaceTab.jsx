                        ...
                        {filteredTopics.map((topic) => (
                            <div key={topic.id} className="transition-colors">
                                        ...
                                        <div className="flex-1">
                                            <div className="flex items-center space-x-3 mb-2">
                                                ...
                                                <span className="text-xs text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded-full">
                                                    {topic.records?.reduce((acc, record) => acc + (record.values?.length || 0), 0)} campos
                                                </span>
                                            </div>
                                ...
                                {expandedTopics.has(topic.id) && (
                                    <div className="pb-2 border-t border-gray-200 dark:border-gray-700">
                                        {viewMode === 'normal' ? (
                                            <div className="overflow-x-auto">
                                                {topic.records && topic.records.length > 0 ? (
                                                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                                        <thead>
                                                            <tr className="bg-gray-50 dark:bg-gray-700">
                                                                {topic.records[0].values.map((field, index) => (
                                                        ...
                                                        <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                                                            {topic.records.map((record, recordIndex) => (
                                                                <tr key={recordIndex} className="hover:bg-slate-700">
                                                                    {record.values.map((field, fieldIndex) => (
                                                                        <td key={fieldIndex} className="px-6 py-3 text-sm text-gray-600 dark:text-gray-300 break-all max-w-md">
                                                                            {field.value}
                                                                            <div className="mt-1">
                                                                                <button
                                                                    ...
                                                                    <td className="px-4 py-3 text-sm">
                                                                        <div className="flex space-x-2">
                                                                            <button
                                                                                onClick={() => copyToClipboard(JSON.stringify(record.values, null, 2), 'Registro copiado como JSON!')}
                                                                                className="text-purple-600 hover:text-purple-800 dark:text-purple-400 dark:hover:text-purple-300"
                                                                                title="Copiar registro como JSON"
                                                                            >
