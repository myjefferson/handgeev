...
<div id="dropdownDotsHorizontal-{{$index}}" class="z-10 hidden bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700 dark:divide-gray-600">
    <ul class="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownMenuIconHorizontalButton-{{$index}}">
        <li>
            <a href={{route('dashboard.experiences.edit', ['id' => $experience->id])}} class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white w-full">Alterar</a>
        </li>
        <li>
...
@include('components.modal-confirm', [
    $index,
    'id' => $experience->id,
    'description' => 'Tem certeza que deseja remover a experiência profissional?',
    'route' => route('dashboard.experiences.delete', ['id' => $experience->id])
])
